...
def text_to_speech(text, language='fr', voice='default'):
    ...
    Voix disponibles avec gTTS:
    - 'com' (default): Voix standard US
    - 'co.uk': Voix britannique
    - 'com.au': Voix australienne
    - 'ca': Voix canadienne
    - 'co.in': Voix indienne
    - 'fr': Voix française
    
    Pour plus d'options (timbre, émotions), utilisez:
    - ElevenLabs API (excellent, payant)
    - Google Cloud TTS (gratuit jusqu'à 1M caractères/mois)
    - Azure TTS (beaucoup de voix)
    - Coqui TTS (open source)
    
    Args:
        text: Le texte à synthétiser
        language: Langue du texte (fr, en, es, de, etc.)
        voice: TLD pour gTTS (com, co.uk, fr, etc.) ou nom de voix pour autres services
    ...
    try:
        ...
        # Mapping des voix vers les TLDs gTTS
        voice_mapping = {
            'default': 'com',
            'us': 'com',
            'uk': 'co.uk',
        ...
        tld = voice_mapping.get(voice, voice)
        
        # Paramètres de vitesse
        slow = voice.endswith('_slow')
        if slow:
            tld = voice_mapping.get(voice.replace('_slow', ''), 'com')
        
        print(f"   🎤 Synthèse: langue={language}, tld={tld}, slow={slow}")
